...
export const imageExists = (
  ...
  const providedImages = fs
    .readdirSync(imagesDir)
    .filter((imgName) => {
      return !fs.lstatSync(path.join(imagesDir, imgName)).isDirectory()
    })
  ...
  if (filename) {
    if (providedImages.includes(<string>filename)) {
      ...
      res
        .status(404)
        .send(
          `must send filename query with one of those provided image filenames ${providedImages}`
        )
    }
  } else {
    res
